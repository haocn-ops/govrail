...
const specs = [
  "tests/browser/api-keys-verification-admin-return.smoke.spec.ts",
...
const smokeExpectations = [
  {
    ...
    requiredPatterns: [
      /api-keys -> verification -> admin keeps readiness return continuity/,
  ...
  },
  {
    ...
    requiredPatterns: [
      /api-keys -> verification -> go-live -> admin keeps readiness browser continuity/,
      /\/api-keys\?source=admin-readiness&week8_focus=credentials&attention_workspace=preview&attention_organization=org_preview&delivery_context=week8&recent_track_key=verification&recent_update_kind=verification&evidence_count=2&recent_owner_label=Ops&recent_owner_display_name=Avery%20Ops&recent_owner_email=avery\.ops%40govrail\.test/,
      /Credential lifecycle/,
      /Admin follow-up context/,
      /Audit export continuity/,
...
test("api-keys verification follow-up batch stays wired into scripts and docs", async () => {
  ...
  assert.equal(
    webPackageJson.scripts?.["test:browser:api-keys-verification-followup"],
    expectedPrebuilt,
  );
  assert.equal(
    webPackageJson.scripts?.["test:browser:api-keys-verification-followup:existing-server"],
    expectedExistingServer,
  );
  assert.equal(
    rootPackageJson.scripts?.["web:test:browser:api-keys-verification-followup"],
    "npm --prefix web run test:browser:api-keys-verification-followup --",
  );
  assert.equal(
    rootPackageJson.scripts?.["web:test:browser:api-keys-verification-followup:existing-server"],
    "npm --prefix web run test:browser:api-keys-verification-followup:existing-server --",
  );

  assert.match(docsReadme, /web:test:browser:api-keys-verification-followup/);
  assert.match(docsReadme, /api-keys -> verification -> admin/);
  assert.match(docsReadme, /api-keys -> verification -> go-live -> admin/);
  assert.match(executionPlan, /api-keys-verification-followup/);
  assert.match(executionPlan, /api-keys -> verification -> go-live -> admin/);
});

for (const spec of smokeExpectations) {
  test(`api-keys verification follow-up smoke keeps ${spec.path} explicit without overstating coverage`, async () => {
