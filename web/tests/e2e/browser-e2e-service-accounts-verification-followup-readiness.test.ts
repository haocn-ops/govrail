...
const smokeExpectations = [
  {
    ...
    requiredPatterns: [
      /service-accounts -> verification -> admin keeps readiness return continuity/,
  ...
  },
  {
    ...
    requiredPatterns: [
      /service-accounts -> verification -> go-live -> admin keeps readiness browser continuity/,
      /\/service-accounts\?source=admin-readiness&week8_focus=credentials&attention_workspace=preview&attention_organization=org_preview&delivery_context=week8&recent_track_key=verification&recent_update_kind=verification&evidence_count=2&recent_owner_label=Ops&recent_owner_display_name=Avery%20Ops&recent_owner_email=avery\.ops%40govrail\.test/,
      /Machine identities/,
      /Admin follow-up context/,
      /Audit export continuity/,
...
for (const spec of smokeExpectations) {
  test(`service-accounts verification follow-up smoke keeps ${spec.path} explicit without overstating coverage`, async () => {
