...
const specs = [
  "tests/browser/playground-verification-admin-return.smoke.spec.ts",
...
test("playground verification follow-up batch stays wired into scripts and docs", async () => {
  ...
  assert.equal(webPackageJson.scripts?.["test:browser:playground-verification-followup"], expectedPrebuilt);
  assert.equal(
    webPackageJson.scripts?.["test:browser:playground-verification-followup:existing-server"],
    expectedExistingServer,
  );
  assert.equal(
    rootPackageJson.scripts?.["web:test:browser:playground-verification-followup"],
    "npm --prefix web run test:browser:playground-verification-followup --",
  );
  assert.equal(
    rootPackageJson.scripts?.["web:test:browser:playground-verification-followup:existing-server"],
    "npm --prefix web run test:browser:playground-verification-followup:existing-server --",
  );
