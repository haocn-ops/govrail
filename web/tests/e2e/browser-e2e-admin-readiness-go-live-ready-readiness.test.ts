...
test("browser readiness go-live-ready smoke keeps go-live/admin continuity explicit without overstating coverage", async () => {
  ...
  assert.match(browserSmokeSpec, /Go-live ready/);
  assert.match(browserSmokeSpec, /Preview Organization/);
  assert.match(browserSmokeSpec, /Preview Workspace/);
  assert.match(browserSmokeSpec, /Drill-down active: Go-live ready/);
