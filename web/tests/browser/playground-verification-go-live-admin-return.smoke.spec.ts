...
test("playground -> verification -> go-live -> admin keeps readiness browser continuity", async ({ page }) => {
  test.slow();

  await page.goto(playgroundEntry);
  ...
  await expect(page.getByRole("heading", { name: "Plan-limit checkpoint" })).toBeVisible();
