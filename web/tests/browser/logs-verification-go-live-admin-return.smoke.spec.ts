...
test("logs -> verification -> go-live -> admin keeps readiness browser continuity", async ({ page }) => {
  test.slow();

  await page.goto(logsEntry);

  await expect(page.getByRole("heading", { name: "Realtime and historical logs" })).toBeVisible();
  ...
  const verificationLink = page.getByRole("link", { name: "Carry proof to verification" }).first();
