...
test("agents -> verification -> go-live -> admin keeps readiness browser continuity", async ({
  page,
}) => {
  test.slow();

  await page.goto(agentsEntry);

  await expect(page.getByRole("heading", { name: "Agent lifecycle management" })).toBeVisible();
  await expect(page.getByText("Admin follow-up context")).toBeVisible();
  await expect(page.getByRole("heading", { name: "Governance continuity" })).toBeVisible();
