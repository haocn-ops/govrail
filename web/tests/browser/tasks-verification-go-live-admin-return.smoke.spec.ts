...
test("tasks -> verification -> go-live -> admin keeps readiness browser continuity", async ({ page }) => {
  test.slow();

  await page.goto(tasksEntry);

  await expect(page.getByRole("heading", { name: "Execution tracking" })).toBeVisible();
  ...
  await expect(page).toHaveURL(/run_id=run_demo_123/);
