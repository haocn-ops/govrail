...
test("admin recent delivery activity branch -> verification -> admin keeps recent-context browser continuity", async ({
  page,
}) => {
  test.slow();
  ...
  expect(verificationUrl.searchParams.get("delivery_context")).toBe("recent_activity");

  const adminQueueReturnLink = page
    .locator('a[href*="queue_returned=1"][href*="delivery_context=recent_activity"]')
