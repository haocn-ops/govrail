...
test("admin attention branch -> verification -> go-live -> admin keeps minimal browser continuity", async ({
  page,
}) => {
  test.slow();
  ...
  const adminQueueReturnLink = page
    .locator('a[href*="queue_returned=1"][href*="queue_surface=go_live"]')
  ...
  await expect(page.getByRole("link", { name: "Clear follow-up return" })).toBeVisible();
});
