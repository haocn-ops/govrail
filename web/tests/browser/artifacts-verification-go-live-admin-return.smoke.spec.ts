...
test("artifacts -> verification -> go-live -> admin keeps readiness browser continuity", async ({
  page,
}) => {
  test.slow();

  await page.goto(artifactsEntry);
  ...
  const verificationLink = page.getByRole("link", { name: "Confirm verification evidence" }).first();
