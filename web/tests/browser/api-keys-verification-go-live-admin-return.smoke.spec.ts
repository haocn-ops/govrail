...
test("api-keys -> verification -> go-live -> admin keeps readiness browser continuity", async ({ page }) => {
  test.slow();

  await page.goto(apiKeysEntry);
  ...
  await expect(page.getByText("Audit export continuity").first()).toBeVisible();
