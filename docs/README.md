...
- [saas_week8_verification_checklist_zh.md](./saas_week8_verification_checklist_zh.md)（新增 delivery tracking panel、attention queue、返回 admin queue、organization drill-down、Week 8 readiness summary 以及 readiness drill-down 使用說明）
  ...
  - 當前驗收邊界仍以 `unit + contract + page + non-browser smoke` 為主，但已新增一組最小 true browser smoke，可透過 `npm run web:test:browser:smoke` 執行：
  ...
  - 這一輪也順手修正了 checklist handoff helper 在 server/client 邊界上的引用方式；完整 browser e2e 仍後置，不應把目前覆蓋寫成真實跨頁點擊與渲染時序已 fully covered。下一小段轉到 `go_live_ready` 與 `demo_run` 的更多 workspace-level action variants，而不是宣稱整條 browser journey 已完整覆蓋。
